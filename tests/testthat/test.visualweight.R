...
test_that("identical observations have visual weight one", {
  expect_equal(visualweight(mtcars[1, ], mtcars[1, ]), 1)
  expect_equivalent(diag(visualweight(mtcars, mtcars)), rep(1, nrow(mtcars)))
  expect_equal(visualweight(mtcars[1:10, ], mtcars[1:5, ]), t(visualweight(
    mtcars[1:5, ], mtcars[1:10, ])))
})
